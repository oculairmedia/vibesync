...
const formulaName = process.env['VIBESYNC_DOGFOOD_FORMULA'] ?? 'dogfood-rig-smoke';
...
const repoRoot = process.env['VIBESYNC_DOGFOOD_REPO_ROOT'] ?? process.cwd();
...
const packRoot = process.env['VIBESYNC_DOGFOOD_PACK_ROOT'] ?? resolveFromAppRoot('packs', 'gastown');
...
console.log(`[rig-dogfood] starting one supervised tick bead=${beadId} agent=${agentId} formula=${formulaName} repoRoot=${repoRoot} packRoot=${packRoot} dryRunMerge=${dryRunMerge}`);
